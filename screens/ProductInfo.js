...
import { darkText, emerald, extraLightBackground, lightBackground, lightText } from '../components/AsosColors';
...
export default class ProductInfo extends Component {
      ...
      <View style={styles.container}>
        <View style={{ position: 'absolute', left: 10, top: 20, zIndex: 2, width: width }}>
          <Animated.View style={{ flexDirection: 'row', justifyContent: 'space-between', opacity: this.animatedOpacity }}>
            <Button type='Icon' icon={'md-arrow-round-back'} onPress={() => this.props.navigation.goBack()} />

            <Button type='Icon' icon={'ios-share'} onPress={() => this.props.navigation.goBack()} />
          </Animated.View>
        </View>
        <ScrollView
  ...
  _renderPage(data, pageID) {
    console.log(data);
    return <Image style={{ width: width, height: 480, resizeMode: 'cover' }} source={{ uri: data }} />;
  }
  onBackButtonPressed() {
